...
TASK_DATABASES = {
    ...
    "Personal": "29820c51aebe80b4abc4c5147ddc288d",  # hours DB
}

# Calendar databases
WORK_CALENDAR_DB = "29520c51aebe80798d10db123c986db0"
PERSONAL_CALENDAR_DB = "2aa20c51aebe80439c52e60fdf45dd31"  # personal calendar DB

# Valid Groups
VALID_GROUPS = {f"Group {i}" for i in range(1, 6)}

# Map word forms to number forms
GROUP_NAME_MAP = {
    "Group One": "Group 1",
    "Group Two": "Group 2",
    "Group Three": "Group 3",
...
def parse_time_range(time_range_str):
    """
    Convert 'Dec 16, 2025 at 1:28 PM - Dec 16, 2025 at 2:28 PM'
    → ISO 8601 with timezone for Notion API
    """
...
def update_hours(database_id, task_title, hours, group_name):
    """Update Hours, Add to Daily Table, and Groups select property"""
    ...
    for page in response["results"]:
        ...
        if task_title.lower() in name.lower():
            ...
            print(f"Updated Hours, checkbox, and group: {name} → {hours} → {group_name}")
            return True
...
def create_calendar_entry(calendar_db_id, group_name, time_range):
    if calendar_entry_exists(calendar_db_id, group_name, time_range):
        print(f"Skipped duplicate calendar entry → {group_name} | {time_range}")
        return
    notion.pages.create(
...
def main():
    # Read input from Shortcuts or terminal
    if len(sys.argv) > 1:
    ...
    group_name = GROUP_NAME_MAP.get(raw_group_name, raw_group_name)  # Normalize
    if group_name not in VALID_GROUPS:
    ...
    for line in task_lines:
        ...
        # Mark which calendar needs an entry
        if updated:
    ...
    # Create calendar entries using the normalized group_name
