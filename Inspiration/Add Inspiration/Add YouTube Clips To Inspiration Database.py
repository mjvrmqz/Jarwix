...
oauth_data = json.loads(os.environ["GOOGLE_OAUTH_CREDENTIALS"])
...
while scanned_videos < MAX_LIKED_SCAN:
    ...
    for item in items:
        ...
        print(
            f"Scanning video {scanned_videos}/{MAX_LIKED_SCAN}: {video_url}"
        )
        ...
        while True:
            ...
            for c in comments_resp.get("items", []):
                ...
                if (
                    author_id == MY_CHANNEL_ID
                    and "🔥" in top_comment["textDisplay"]
                ):
            ...
            next_comment_token = comments_resp.get(
                "nextPageToken"
            )
    ...
    if (
        not next_page_token
        or scanned_videos >= MAX_LIKED_SCAN
    ):
        break
