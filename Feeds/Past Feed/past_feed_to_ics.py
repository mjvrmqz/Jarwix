...
def extract_props(page):
    ...
    def get_title():
        try: return props[" Calendar"]["title"][0]["plain_text"]
        except (KeyError, IndexError): return None
    def get_select(field):
        try: return props[field]["select"]["name"]
        except (KeyError, TypeError): return None
    def get_multi_select(field):
        try: return [o["name"] for o in props[field]["multi_select"]]
    ...
    def get_number(field):
        try: return props[field]["number"]
        except (KeyError, TypeError): return None
    def get_date(field):
        try: return props[field]["date"]
        except (KeyError, TypeError): return None
    return {
        "title": get_title(), "time": get_date("Time"),
        "actionable_steps": get_rich_text("Actionable Steps"),
        "done": get_select("Done?"), "reflection": get_select("Reflection"),
        "priority": get_select("Priority"), "why": get_select("Why?"),
        "hours": get_number("Hours"), "block": get_multi_select("Block"),
    }

def build_page_payload(props):
    properties = {
        " Calendar": {"title": [{"type": "text", "text": {"content": props["title"]}}]},
        "Actionable Steps": {"rich_text": [{"type": "text", "text": {"content": props["actionable_steps"]}}] if props["actionable_steps"] else []},
    }
    if props["time"]: properties["Time"] = {"date": props["time"]}
    if props["done"]: properties["Done?"] = {"select": {"name": props["done"]}}
    if props["reflection"]: properties["Reflection"] = {"select": {"name": props["reflection"]}}
    if props["priority"]: properties["Priority"] = {"select": {"name": props["priority"]}}
    if props["why"]: properties["Why?"] = {"select": {"name": props["why"]}}
    if props["hours"] is not None: properties["Hours"] = {"number": props["hours"]}
    if props["block"]: properties["Block"] = {"multi_select": [{"name": n} for n in props["block"]]}
    return {"parent": {"database_id": PAST_FEED_DB}, "properties": properties}
...
def create_ics(pages, output_file="Feeds/Past Feed/Past Feed.ics"):
    cal = Calendar()
    for page in pages:
        try:
            title = page["properties"][" Calendar"]["title"][0]["plain_text"]
            time_prop = page["properties"]["Time"]["date"]
            if not time_prop: continue
...
def main():
    ...
    skipped = created = deleted = 0
    for page in all_pages:
        props = extract_props(page)
        if not props["title"]: skipped += 1; continue
